### obtained from https://github.com/igorbrigadir/DownloadConceptualCaptions
...
def _file_name(row):
    ...
    if row['folder'] == 'validation':
        sub_folder = 'val'
    elif row['folder'] == 'training':
        sub_folder = 'train_gist'
    
    return "%s/%s_%s.jpg" % (folder+sub_folder, row.name, (zlib.crc32(row['url'].encode('utf-8')) & 0xffffffff))
...
df = open_tsv(folder+"Validation_GCC-1.1.0-Validation.tsv", data_name)
...
data_name = "training"
df = open_tsv(folder+"Train_GCC-training.tsv",data_name)
df_multiprocess(df=df, processes=num_processes, chunk_size=images_per_part, func=download_image, dataset_name=data_name)
df = df_from_shelve(chunk_size=images_per_part, func=download_image, dataset_name=data_name)
df.to_csv("downloaded_%s_report.tsv.gz" % data_name, compression='gzip', sep='\t', header=False, index=False)
print("Saved.")
